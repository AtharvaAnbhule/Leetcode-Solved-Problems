...
public:
    ...
    static long long kthLargestLevelSum(TreeNode* root, int k) {
        ...
        f(root, 0, sum, sz);
        if (sz<k) return -1;
        nth_element(sum, sum+(k-1), end(sum), greater<>()); // nth_element rearranges the array such that the element at index k-1 is in its final sorted position, and all elements before it are larger or equal.
        return sum[k-1];
    }
};
